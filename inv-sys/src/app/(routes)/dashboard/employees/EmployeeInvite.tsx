...
export default function InviteForm({ onClose }: InviteFormProps) {
  ...
  const [role, setRole] = useState<Role>(Role.Vendedor)
      ...
      <div className="space-y-6">
        ...
        <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg p-3">
          <p className="text-yellow-300 text-sm">
            <strong>Importante:</strong> Este código expira en 7 días. Compártelo con el empleado para que complete su
            registro.
          </p>
        </div>
        ...
        <div className="flex justify-end">
          <button
            onClick={onClose}
            className="px-6 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 transform hover:scale-105"
          >
            Cerrar
          </button>
        </div>
    ...
    <form onSubmit={handleInviteSubmit} className="space-y-6">
      ...
      <div className="space-y-4">
        <div>
          ...
          <input
            ...
            required
            className="w-full px-4 py-3 bg-gray-800/50 backdrop-blur-sm border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
        ...
        <div>
          ...
          <select
            ...
            className="w-full px-4 py-3 bg-gray-800/50 backdrop-blur-sm border border-gray-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
          >
            {Object.values(Role).map((roleOption) => (
              <option key={roleOption} value={roleOption} className="bg-gray-800">
                {roleOption.charAt(0).toUpperCase() + roleOption.slice(1)} (ID: {roleNumbers[roleOption]})
              </option>
            ))}
          </select>
        </div>
      ...
      <div className="flex justify-end space-x-3 pt-4">
        ...
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-lg hover:from-green-600 hover:to-emerald-700 transition-all duration-200 transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
        >
